...
def serialize_quantized_cell(d,gx,gy,thr,minq):
    corners,_=relevant(d,gx,gy)
    mask=0
    for v in corners: mask|=1<<v
    out=bytearray((mask&255,(mask>>8)&255))
    for v in corners:
        dep,_=corner_quant_depth(d,v,gx,gy,thr,minq)
        if dep is None:
            out.append(0xff)
        ...
        for yy in range(n):
          for xx in range(n):
            rec=quant_leaf_record(d,v,gx,gy,xx*step,(xx+1)*step,yy*step,(yy+1)*step)
            if rec is None: raise RuntimeError(f"quant record overflow cell={gx},{gy} corner={v} depth={dep}")
            base,sx,sy=rec
            out.extend((base&255,(base>>8)&15,sx&255,sy&255))
    return bytes(out)


def c_u8_array(name,data,cols=16):
    lines=[f"static const uint8_t {name}[{len(data)}] = {{"]
    for i in range(0,len(data),cols):
        lines.append("    "+", ".join(str(x) for x in data[i:i+cols])+",")
    lines.append("};")
    return "\n".join(lines)


def c_u16_array(name,data,cols=12):
...
def emit_runtime_banks(d,outdir,thr,minq,rows_per_bank):
    ...
    for bank in range(bank_count):
        ...
        for gy in range(y0,y1):
          for gx in range(GRID_W):
            block=serialize_quantized_cell(d,gx,gy,thr,minq)
            max_cell=max(max_cell,len(block))
            payload.extend(block);offsets.append(len(payload))
        bank_sizes.append((bank,y0,y1,len(payload),len(offsets)*2))
        src=f"""/* GENERATED by local_projection_field_poc.py --emit-dir. */
#include <stdint.h>
#include <string.h>
#include <gbdk/platform.h>
#pragma bank 255
BANKREF(tilesector_polar_proj_bank{bank})

{c_u16_array("k_proj_off",offsets)}

{c_u8_array("k_proj_data",payload)}

void tsp_polar_proj_load_bank{bank}(uint16_t cell, uint8_t *dst) BANKED {{
    uint16_t a=k_proj_off[cell], b=k_proj_off[cell+1u];
    if(b>a) memcpy(dst,&k_proj_data[a],(uint16_t)(b-a));
}}
"""
        (outdir/f"tilesector_polar_proj_bank{bank}.c").write_text(src)

    hdr=[
        "/* GENERATED local projection runtime metadata. */",
        "#ifndef TILESECTOR_POLAR_PROJECTION_META_H",
        "#define TILESECTOR_POLAR_PROJECTION_META_H",
        "#include <stdint.h>",
        "#include <gbdk/platform.h>",
        f"#define TSPF_PROJ_BANK_COUNT {bank_count}u",
        f"#define TSPF_PROJ_ROWS_PER_BANK {rows_per_bank}u",
        f"#define TSPF_PROJ_MAX_CELL_BYTES {max_cell}u",
    ]
    for bank in range(bank_count):
        hdr.append(f"void tsp_polar_proj_load_bank{bank}(uint16_t cell, uint8_t *dst) BANKED;")
    hdr.append("#endif")
    (outdir/"tilesector_polar_projection_meta.h").write_text("\n".join(hdr)+"\n")

    print(f"EMIT projection banks: threshold={thr:g} Q12 rows/bank={rows_per_bank} banks={bank_count} max-cell={max_cell}")
    for bank,y0,y1,payload,offs in bank_sizes:
        print(f"  bank-src {bank}: rows {y0:02d}-{y1-1:02d} payload={payload} offsets={offs} data-total={payload+offs}")
    return bank_sizes,max_cell
