...
static const uint8_t k_square16[16] = {
    49u, 36u, 25u, 16u, 9u, 4u, 1u, 0u,
    1u, 4u, 9u, 16u, 25u, 36u, 49u, 64u
};

static int16_t fx_abs16(int16_t v) { return v < 0 ? (int16_t)-v : v; }

static uint8_t mod3_u8(uint8_t v) {
    v = (uint8_t)((v & 0x0fu) + (v >> 4));
    v = (uint8_t)((v & 0x03u) + (v >> 2));
    v = (uint8_t)((v & 0x03u) + (v >> 2));
    if (v >= 3u) v = (uint8_t)(v - 3u);
    if (v >= 3u) v = (uint8_t)(v - 3u);
    return v;
}

static uint8_t mod3_sum(uint16_t v) {
    uint8_t r = (uint8_t)(mod3_u8((uint8_t)v) + mod3_u8((uint8_t)(v >> 8)));
    if (r >= 3u) r = (uint8_t)(r - 3u);
    return r;
}
...
uint8_t fx_dither_on(uint8_t x, uint8_t y, uint8_t phase, uint8_t numerator) {
    ...
    if (!numerator) return 0u;
    sum = (uint16_t)x + ((uint16_t)y << 1) + phase;
    bucket = mod3_sum(sum);
...
void fx_tile_draw_ring(uint8_t *tile, uint8_t radius, uint8_t color, uint8_t phase) {
    int8_t x, y;
    uint8_t r = radius > 7u ? 7u : radius;
    uint8_t r2 = (uint8_t)(r * r);
    uint8_t inner_r = r ? (uint8_t)(r - 1u) : 0u;
    uint8_t inner = (uint8_t)(inner_r * inner_r);
    for (y = 0; y < 8; ++y) {
        for (x = 0; x < 8; ++x) {
            int8_t dx = (int8_t)(x - 3);
            int8_t dy = (int8_t)(y - 3);
            uint8_t d2 = (uint8_t)(dx * dx + dy * dy);
...
void fx_tile_draw_ring16_quadrant(uint8_t *tile, uint8_t quadrant, uint8_t radius, uint8_t color, uint8_t phase) {
    uint8_t x, y;
    uint8_t ox = (quadrant & 1u) ? 8u : 0u;
    uint8_t oy = (quadrant & 2u) ? 8u : 0u;
    uint8_t r = radius > 15u ? 15u : radius;
    uint8_t inner_r = r > 1u ? (uint8_t)(r - 2u) : 0u;
    uint16_t r2 = (uint16_t)r * r;
    uint16_t inner = (uint16_t)inner_r * inner_r;
    fx_tile_clear(tile);
    for (y = 0u; y < 8u; ++y) {
        for (x = 0u; x < 8u; ++x) {
            uint8_t gx = (uint8_t)(x + ox);
            uint8_t gy = (uint8_t)(y + oy);
            uint16_t d2 = (uint16_t)k_square16[gx] + k_square16[gy];
            if (d2 <= r2 && d2 >= inner && fx_dither_on(gx, gy, phase, 2u))
                fx_tile_set_pixel(tile, x, y, color);
        }
    }
}
