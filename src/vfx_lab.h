...
typedef struct {
    ...
    uint8_t hold_frames;
    uint8_t phase_tick;
    uint8_t duty_numer; /* 3=solid, 2=2/3, 1=1/3 */
    int16_t x0, y0;     /* world pixels */
    int16_t x1, y1;
...
typedef struct {
    ...
    int16_t impact_x, impact_y; /* world pixels */
    uint8_t impact_active;

    int16_t ring_x, ring_y;     /* world pixels */
    uint8_t ring_active;
    uint8_t ring_radius;
    uint8_t ring_phase;
