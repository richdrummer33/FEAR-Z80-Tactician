...
/* Four-byte validity bitset for the 32 possible compact vertices.  The exact
...
static uint16_t bearing_vertex_q12(uint8_t vid, const TSPState *s)
{
...
#if defined(TSPF_E1M1_FRONT_ENVELOPE_EXACT)
    {
        ...
        uint8_t bm=k_nt_mask8[vid&7u];
        if(!(g_corner_bearing_valid[bi]&bm))
        {
#if defined(__SDCC) && TSPF_E1M1_LOCAL_BEARING_FIELD
            if(!(g_e1env_lbf_cell[bi]&bm))
            {
...
#endif
#if TSPF_DIRECT_MIXED
        /* Both spans are simultaneously visible and physically connected.
         * The optional direct-mode affordance omits that INTERNAL crease while
         * preserving the exact ownership/top-edge handoff. For dx!=0 retain
         * the right coarse border transactionally until mixed preparation
         * succeeds; for dx==0 there is no mixed tile, so both coarse borders
...
static void draw_run(uint16_t *out, TSPColumn *cols, const PolarRun *r, const TSPState *s)
{
       ...
       )
    {
        ...
        g_polar_run_left_real=(uint8_t)(r->left_real &&
            !(g_tspf_mixed_border_clear_sid[r->sid]&1u));
        g_polar_run_right_real=(uint8_t)(r->right_real &&
            !(g_tspf_mixed_border_clear_sid[r->sid]&2u));
#else
...
void tsp_polar_render(const TSPState *s, uint16_t out_map[TSP_MAP_CELLS], TSPColumn cols[TSP_COLS]) BANKED
{
...
#if defined(TSPF_E1M1_FULL_ONLY)
#if defined(TSPF_E1M1_FRONT_ENVELOPE_EXACT)
    /* A yaw-only update does not change world-space vertex bearings.
     * Translation invalidates the compact four-byte bitset once; the envelope
     * then lazily repopulates only vertices it actually touches. */
    if(!g_corner_bearing_pos_valid ||
       ...
       s->y_q4!=g_corner_bearing_y_q4){
        ...
        g_corner_bearing_valid[0]=0u;
        g_corner_bearing_valid[1]=0u;
        g_corner_bearing_valid[2]=0u;
        g_corner_bearing_valid[3]=0u;
    }
#else
...
#if defined(TSPF_E1M1_FRONT_ENVELOPE)
    {
        ...
        if(n!=0xffu){
            ...
            /* Camera yaw can lie in a narrow envelope span which owns no
