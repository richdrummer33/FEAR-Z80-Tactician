/*
 * Lit-room + transparent FLAT hero LOD runtime proof.
 *
 * Sibling of main_hero_sprite_lod_room_gg.c, which proved the originally
 * proposed small nested core+refinement scheme (16+16 patterns, 1 KiB).
 * Measurement afterward showed a single vocabulary trained jointly on mid
 * and far demands beats two independently-sized dictionaries of the same
 * total budget at every size tested -- the two distance bands share enough
 * structure that splitting the budget between them wastes it. See
 ...
 * 256 so VRAM 0x0000..0x1FFF is never used by room patterns (192 hero
 * patterns reserved at the top of the shared pattern table).
 *
 * Hero sprites (assuming the normal GG/SMS sprite pattern base at 0x2000):
 *   sprite tile 0..191 => VRAM 0x2000..0x37ff, one shared vocabulary
 *
 * Runtime intent:
 *   - the whole vocabulary is loaded once, with the display blanked;
 *   - angle changes AND mid/far distance changes both rewrite sprite
 *     attributes only -- there is no second load stage, because there is
...
void main(void){
    ...
    /* Cold room frame and the whole shared hero vocabulary are loaded with
     * the display blanked, so neither costs visible-time bandwidth. Unlike
     * the nested proof there is no second load stage: the same 192 patterns
     * already cover both distance bands. */
    ...
    for(;;){
        ...
        /* Manual inspection controls. Crossing bands is a sprite-attribute
         * rewrite exactly like an angle change -- no load call at all. */
        if(pressed&J_LEFT){
        ...
        if(pressed&J_UP){g_hero_lod_band=HERO_LOD_MID_BAND;apply_hero();}
        if(pressed&J_DOWN){g_hero_lod_band=HERO_LOD_FAR_BAND;apply_hero();}

        /* No input is needed for CI or browser inspection: orbit the object
         * continuously and periodically cross the mid/far boundary. */
        ...
        if(++band_counter>=AUTO_BAND_FRAMES){
            band_counter=0u;
            g_hero_lod_band=(g_hero_lod_band==HERO_LOD_FAR_BAND)?
                HERO_LOD_MID_BAND:HERO_LOD_FAR_BAND;
            apply_hero();
        }
