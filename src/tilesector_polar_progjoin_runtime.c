...
static uint8_t record_byte(uint16_t off)
{
    ...
    {
        SWITCH_ROM2(PJ_BANK_REC0);
        return gg_pj_records0[off];
    }
    SWITCH_ROM2(PJ_BANK_REC1);
                                   ...
                                   int16_t iq0, TSPProgjoinRunPlan *out)
{
    uint8_t left, i;
    int16_t iq;
    if (!out || ncol == 0u || ncol > TSP_PROGJOIN_MAX_RUN_COLS) return 0u;
    out->count = 0u; left = ncol; iq = iq0; i = 0u;
    while (left)
    {
        uint8_t want = (left > TSP_PROGJOIN_CHUNK_COLS) ? TSP_PROGJOIN_CHUNK_COLS : left;
        if (i >= TSP_PROGJOIN_MAX_CHUNKS) return 0u;
        if (!tsp_progjoin_dispatch_body(step, fam, want, iq, &out->bodies[i]))
        { out->count = 0u; return 0u; }
        out->wants[i] = want;
        iq = (int16_t)(iq + (int16_t)((int16_t)want * step));
        left = (uint8_t)(left - want); ++i;
    }
    out->count = i; return 1u;
}
                               ...
                               int16_t *cursor)
{
    ...
    if (!basep || !dst || !cursor || body.off >= PJ_BANK_BYTES || dst_bytes < 2u) return 0u;
    p = basep + body.off; n = *p++;
    for (i = 0u; i != n; ++i)
    {
        uint16_t word; int16_t delta; int16_t c = *cursor;
        if (c < 0 || c >= (int16_t)(dst_bytes - 1u)) return 0u;
        word = (uint16_t)p[0] | ((uint16_t)p[1] << 8);
        delta = (int16_t)((uint16_t)p[2] | ((uint16_t)p[3] << 8)); p += 4;
        dst[(uint16_t)c] = (uint8_t)word; dst[(uint16_t)c + 1u] = (uint8_t)(word >> 8);
...
{
    uint8_t i;
    if (!plan || plan->count == 0u || plan->count > TSP_PROGJOIN_MAX_CHUNKS) return 0u;
    for (i = 0u; i != plan->count; ++i)
        if (!tsp_progjoin_play_body(plan->bodies[i], dst, dst_bytes, cursor)) return 0u;
    return 1u;
}
                            ...
                            int16_t *cursor)
{
    ...
    *cursor = (int16_t)(*cursor + step_bytes);
    ...
    {
        *row = (int8_t)(*row + 1);
        if (rb == 7u) { *rowbit = 0u; *cov = (int8_t)(*cov + 1); }
        else *rowbit = (uint8_t)(rb + 1u);
        return 1u;
    }
    *col = (uint8_t)(*col + 1u);
    if (step_bytes == 2) { *cov = (int8_t)(*cov + 3); return 1u; }
    if (step_bytes == -38)
    { *row = (int8_t)(*row - 1); *cov = (int8_t)(*cov + ((rb == 0u) ? 2 : 3)); *rowbit = (uint8_t)((rb + 7u) & 7u); return 1u; }
    if (step_bytes == -78)
    { *row = (int8_t)(*row - 2); *cov = (int8_t)(*cov + ((rb < 2u) ? 2 : 3)); *rowbit = (uint8_t)((rb + 6u) & 7u); return 1u; }
    if (step_bytes == -118)
    { *row = (int8_t)(*row - 3); *cov = (int8_t)(*cov + ((rb < 3u) ? 2 : 3)); *rowbit = (uint8_t)((rb + 5u) & 7u); return 1u; }
    return 0u;
}
...
uint8_t tsp_progjoin_play_plan_gated(const TSPProgjoinRunPlan *plan,
                                     ...
                                     uint8_t first_col)
{
    uint8_t pi; int8_t row, cov; uint8_t col, rowbit; int16_t cursor;
    if (!plan || !dst_words || !coverage60 || !row_min18 || !row_max18 ||
        plan->count == 0u || plan->count > TSP_PROGJOIN_MAX_CHUNKS || first_col >= 20u) return 0u;
    row = first_row; col = first_col; rowbit = (uint8_t)first_row & 7u;
    cov = (int8_t)((int8_t)(first_col + first_col + first_col) + ((first_row >= 0) ? (first_row >> 3) : -1));
    ...
    for (pi = 0u; pi != plan->count; ++pi)
    {
        ...
        const uint8_t *basep = body_base(body.bank); const uint8_t *p; uint8_t n, i;
        if (!basep || body.off >= PJ_BANK_BYTES) return 0u;
        p = basep + body.off; n = *p++;
        for (i = 0u; i != n; ++i)
        {
            uint16_t word = (uint16_t)p[0] | ((uint16_t)p[1] << 8);
            int16_t delta = (int16_t)((uint16_t)p[2] | ((uint16_t)p[3] << 8));
            int16_t step_bytes = (int16_t)(1 + delta); p += 4;
            if (row >= 0 && row < 18 && col < 20u)
            {
                uint8_t mask = (uint8_t)(1u << rowbit); uint8_t ci = (uint8_t)cov;
                if ((coverage60[ci] & mask) == 0u)
                {
                    ...
                    {
                        dst_words[wi] = word;
                        if (row_min18[(uint8_t)row] == 0xFFu || col < row_min18[(uint8_t)row]) row_min18[(uint8_t)row] = col;
                        if (col > row_max18[(uint8_t)row]) row_max18[(uint8_t)row] = col;
                    }
                }
            }
