...
int main(int argc, char** argv) {
    ...
    const bool have_mixed_diag =
        ...
        (find_symbol(noi, "_g_tspf_mixed_collinear_tile_noops", s_mix_collinear_noop) || find_symbol(noi, "g_tspf_mixed_collinear_tile_noops", s_mix_collinear_noop)) &&
        ...
        if (lc != last_loop) {
            if (last_loop != 0xFFFFu) {
                ...
                if (have_mixed_diag) {
                    ...
                    cur.mix_collinear_noop=mem->DebugRetrieve(s_mix_collinear_noop);
    ...
    if (csv) {
        ...
        std::fprintf(csv, ",x_q4,y_q4,z_q4,yaw,map_fnv64,dirty_rows_pending,vblank_bursts,vblank_missed,join_anchor_count,join_anchor_max_px,join_anchor_sum_px,mixed_patterns,mixed_patches,mixed_skip,mixed_fallback,mixed_chain,mixed_unsupported,mixed_connected_elided,mixed_collinear_collapsed,mixed_collinear_tile_noops,mixed_silhouette_lines,mixed_any,mixed_pending,mixed_events,mixed_overflow\n");
        for (size_t i = 0; i < frames.size(); ++i) {
            ...
            std::fprintf(csv, ",%d,%d,%d,%u,%016llx,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
                ...
                (unsigned)f.mix_silhouette,(unsigned)f.mix_any,
    ...
    if(have_mixed_diag){
        ...
        std::printf("direct mixed: exact_updates=%.1f%% events mean=%.2f p95=%.0f worst=%.0f "
                    ...
                    "silhouette_lines=%u pending_updates=%.1f%% overflows=%u\n",
