...
int main(int argc, char** argv) {
    ...
    u16 s_mix_elided=0, s_mix_collinear=0, s_mix_silhouette=0;
    ...
    const bool have_mixed_diag =
        ...
        (find_symbol(noi, "_g_tspf_mixed_collinear_collapsed", s_mix_collinear) || find_symbol(noi, "g_tspf_mixed_collinear_collapsed", s_mix_collinear)) &&
    ...
    struct Frame {
        ...
        uint8_t mix_elided,mix_collinear,mix_silhouette;
        ...
        if (lc != last_loop) {
            if (last_loop != 0xFFFFu) {
                ...
                if (have_mixed_diag) {
                    ...
                    cur.mix_collinear=mem->DebugRetrieve(s_mix_collinear);
    ...
    if (csv) {
        ...
        std::fprintf(csv, ",x_q4,y_q4,z_q4,yaw,map_fnv64,dirty_rows_pending,vblank_bursts,vblank_missed,join_anchor_count,join_anchor_max_px,join_anchor_sum_px,mixed_patterns,mixed_patches,mixed_skip,mixed_fallback,mixed_chain,mixed_unsupported,mixed_connected_elided,mixed_collinear_collapsed,mixed_silhouette_lines,mixed_any,mixed_pending,mixed_events,mixed_overflow\n");
        for (size_t i = 0; i < frames.size(); ++i) {
            ...
            std::fprintf(csv, ",%d,%d,%d,%u,%016llx,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
                ...
                (unsigned)f.mix_elided,(unsigned)f.mix_collinear,(unsigned)f.mix_silhouette,(unsigned)f.mix_any,
    ...
    if(have_mixed_diag){
        ...
        unsigned elided=0u,collinear=0u,silhouette=0u;
        ...
        for(const auto& f:frames){
            ...
            elided+=f.mix_elided;collinear+=f.mix_collinear;silhouette+=f.mix_silhouette;
        ...
        std::printf("direct mixed: exact_updates=%.1f%% events mean=%.2f p95=%.0f worst=%.0f "
                    ...
                    "connected_lines_elided=%u collinear_joins_collapsed=%u silhouette_lines=%u pending_updates=%.1f%% overflows=%u\n",
                    100.0*exact/frames.size(),em,pct(events,.95),pct(events,1.0),
                    pm,pct(pats,.95),pct(pats,1.0),wm,pct(patches,.95),pct(patches,1.0),
                    fallback,chain,unsupported,elided,collinear,silhouette,
