 ...
 */\n`;
out+=`#define DOOMGUY_SEAM_COMPONENT_COUNT ${comps.length}u\n`;
out+=`typedef struct RMBGeneratedSeamLayer { uint8_t rank,quarters; const int16_t *xyz; uint16_t vertex_count; const uint16_t *idx; uint16_t triangle_count; } RMBGeneratedSeamLayer;\n`;
for(let i=0;i<layers.length;i++){const l=layers[i],nme=`doomguy_seam_l${i}`;out+=carr('int16_t',nme+'_xyz_q8',l.xyz)+carr('uint16_t',nme+'_indices',l.idx);}
out+='static const RMBGeneratedSeamLayer doomguy_seam_layers[] = {\n';
for(let i=0;i<layers.length;i++){const l=layers[i],nme=`doomguy_seam_l${i}`;out+=`  {${l.rank}u,${l.quarters}u,${nme}_xyz_q8,${l.verts}u,${nme}_indices,${l.tris}u},\n`;}
out+='};\n#define DOOMGUY_SEAM_LAYER_COUNT ((uint8_t)(sizeof(doomguy_seam_layers)/sizeof(doomguy_seam_layers[0])))\n';
