...
/*
 * Crease field. Measured on the welded FULL-RESOLUTION source, then stencilled
 * onto the decimated visual shell. Measuring it on the shell instead would be
 * measuring a surface whose folds decimation has already removed.
 */
...
const srcQ8Pos=(()=>{
  const out=new Float64Array(srcWelded.vertexCount*3);
  for(let i=0;i<srcWelded.vertexCount;++i){
    const q=mapAxes([srcWelded.xyz[i*3],srcWelded.xyz[i*3+1],srcWelded.xyz[i*3+2]],up);
...
const visualPos=(()=>{
  const out=new Float64Array(visualGeom.vertexCount*3);
  for(let i=0;i<visualGeom.vertexCount;++i)
    for(let a=0;a<3;++a)out[i*3+a]=visualQ8[i*3+a]/256;
  return out;
})();
const visualConc=transferToShell(srcQ8Pos,srcConc,srcWelded.vertexCount,
                                 visualPos,visualGeom.vertexCount,recessRadius);
/* Quantize to uint8 over the observed positive range. Negative (convex) is
 * clamped to zero: only folds are of interest. */
const concMax=visualConc.reduce((m,v)=>v>m?v:m,0)||1;
const visualRecess=new Uint8Array(visualGeom.vertexCount);
for(let i=0;i<visualGeom.vertexCount;++i){
  const v=visualConc[i]>0?visualConc[i]/concMax:0;
  visualRecess[i]=Math.max(0,Math.min(255,Math.round(v*255)));
}
const recessStats=(()=>{
