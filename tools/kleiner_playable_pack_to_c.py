#!/usr/bin/env python3
"""
Pack the random-access Kleiner inspection lattice into autobanked GBDK C.
...
def parse_pack(path):
    ...
    if data[:4] != b"KLP1":
        raise SystemExit("bad Kleiner playable pack magic")
    if u16(data, 4) != 1:
        raise SystemExit("unsupported Kleiner playable pack version")
...
def emit_bank(outdir, bank_index, first, states, meta):
    ...
    s = f"""/* GENERATED Kleiner playable state bank {bank_index}. */
#include <stdint.h>
...
#pragma bank 255
BANKREF(kleiner_playable_bank{bank_index})
...
uint16_t kleiner_play_patterns_bank{bank_index}(uint16_t local) BANKED {{
    const uint8_t *p=state_ptr(local);
    if(!p)return 0u;
    return (uint16_t)p[0]|((uint16_t)p[1]<<8);
}}

void kleiner_play_upload_bank{bank_index}(uint16_t local,uint8_t pool,
    ...
    base=pool?KLEINER_PLAY_POOL_B_BASE:KLEINER_PLAY_POOL_A_BASE;
    p+=2u+KLEINER_PLAY_MAP_BYTES+(uint32_t)first*32u;
    set_bkg_4bpp_data((uint16_t)(base+first),(uint8_t)count,p);
}}

void kleiner_play_name_bank{bank_index}(uint16_t local,uint8_t pool) BANKED {{
    const uint8_t *p=state_ptr(local);
    uint16_t i,n,shift=pool?KLEINER_PLAY_POOL_SHIFT:0u;
    uint8_t row;
    if(!p)return;
    n=(uint16_t)p[0]|((uint16_t)p[1]<<8);
    (void)n;
    ...
    for(i=0u;i<TSP_MAP_CELLS;++i){{
        ...
        p+=2u;
        if(id>=KLEINER_PLAY_POOL_A_BASE &&
           id<KLEINER_PLAY_POOL_A_BASE+KLEINER_PLAY_POOL_SIZE)
...
"""
    (outdir / f"kleiner_playable_bank{bank_index}.c").write_text(s)
...
def emit_dispatch_group(outdir, group_index, indexed_chunks):
    ...
    for ordinal, (bank_index, first, states) in enumerate(indexed_chunks):
        ...
        decl += [
            f"uint16_t kleiner_play_patterns_bank{bank_index}(uint16_t local) BANKED;",
            f"void kleiner_play_upload_bank{bank_index}(uint16_t local,uint8_t pool,uint16_t first,uint16_t count) BANKED;",
            f"void kleiner_play_name_bank{bank_index}(uint16_t local,uint8_t pool) BANKED;",
        ]
        pattern_cases.append(
            f"    {kw}(state<{end}u)return kleiner_play_patterns_bank{bank_index}((uint16_t)(state-{first}u));")
        upload_cases.append(
            f"    {kw}(state<{end}u){{kleiner_play_upload_bank{bank_index}((uint16_t)(state-{first}u),pool,first,count);return;}}")
        name_cases.append(
            f"    {kw}(state<{end}u){{kleiner_play_name_bank{bank_index}((uint16_t)(state-{first}u),pool);return;}}")

    s = f"""/* GENERATED Kleiner playable dispatch group {group_index}. */
#include <stdint.h>
#include <gbdk/platform.h>
#include "kleiner_playable_meta.h"

#pragma bank 255
BANKREF(kleiner_playable_dispatch_group{group_index})

{chr(10).join(decl)}

uint16_t kleiner_play_patterns_group{group_index}(uint16_t state) BANKED {{
{chr(10).join(pattern_cases)}
    return 0u;
}}

void kleiner_play_upload_group{group_index}(uint16_t state,uint8_t pool,
                                        uint16_t first,uint16_t count) BANKED {{
{chr(10).join(upload_cases)}
}}

void kleiner_play_name_group{group_index}(uint16_t state,uint8_t pool) BANKED {{
{chr(10).join(name_cases)}
}}
"""
    (outdir / f"kleiner_playable_dispatch_group{group_index}.c").write_text(s)
...
def emit_dispatch(outdir, chunks, meta, lut, dictionary):
    ...
    for gi, indexed in enumerate(groups):
        ...
        group_decl += [
            f"uint16_t kleiner_play_patterns_group{gi}(uint16_t state) BANKED;",
            f"void kleiner_play_upload_group{gi}(uint16_t state,uint8_t pool,uint16_t first,uint16_t count) BANKED;",
            f"void kleiner_play_name_group{gi}(uint16_t state,uint8_t pool) BANKED;",
        ]
        pattern_cases.append(
            f"    {kw}(state<{end}u)return kleiner_play_patterns_group{gi}(state);")
        upload_cases.append(
            f"    {kw}(state<{end}u){{kleiner_play_upload_group{gi}(state,pool,first,count);return;}}")
        name_cases.append(
            f"    {kw}(state<{end}u){{kleiner_play_name_group{gi}(state,pool);return;}}")
        emit_dispatch_group(outdir, gi, indexed)

    h = f"""#ifndef KLEINER_PLAYABLE_META_H
#define KLEINER_PLAYABLE_META_H
#include <stdint.h>
#include <gbdk/platform.h>

#define KLEINER_PLAY_GRID_W {meta["grid_w"]}u
#define KLEINER_PLAY_GRID_H {meta["grid_h"]}u
#define KLEINER_PLAY_YAWS {meta["yaws"]}u
#define KLEINER_PLAY_POSITION_COUNT {meta["positions"]}u
#define KLEINER_PLAY_STATE_COUNT {meta["states"]}u
#define KLEINER_PLAY_ORIGIN_X {meta["origin_x"]}
#define KLEINER_PLAY_ORIGIN_Y {meta["origin_y"]}
#define KLEINER_PLAY_STEP {meta["step"]}u
#define KLEINER_PLAY_EYE_Z {meta["eye_z"]}u
#define KLEINER_PLAY_POOL_A_BASE {meta["pool_a"]}u
#define KLEINER_PLAY_POOL_B_BASE {meta["pool_b"]}u
#define KLEINER_PLAY_POOL_SHIFT (KLEINER_PLAY_POOL_B_BASE-KLEINER_PLAY_POOL_A_BASE)
#define KLEINER_PLAY_POOL_SIZE {meta["pool_size"]}u
#define KLEINER_PLAY_DICT_BASE {meta["dict_base"]}u
#define KLEINER_PLAY_DICT_COUNT {meta["dict_count"]}u
#define KLEINER_PLAY_MAP_BYTES {MAP_BYTES}u

uint8_t kleiner_play_position_ordinal(uint8_t ix,uint8_t iy) BANKED;
uint16_t kleiner_play_state_patterns(uint16_t state) BANKED;
void kleiner_play_upload_state(uint16_t state,uint8_t pool,
                            uint16_t first,uint16_t count) BANKED;
void kleiner_play_apply_name(uint16_t state,uint8_t pool) BANKED;
void kleiner_play_load_dictionary(void) BANKED;
#endif
"""
    (outdir / "kleiner_playable_meta.h").write_text(h)

    s = f"""/* GENERATED top-level Kleiner playable dispatcher + dictionary. */
#include <stdint.h>
#include <gbdk/platform.h>
#include "tilesector_polar.h"
#include "kleiner_playable_meta.h"
...
#pragma bank 255
BANKREF(kleiner_playable_dispatch)
...
uint8_t kleiner_play_position_ordinal(uint8_t ix,uint8_t iy) BANKED {{
    if(ix>=KLEINER_PLAY_GRID_W||iy>=KLEINER_PLAY_GRID_H)return 0xffu;
    return k_grid_lut[(uint16_t)iy*KLEINER_PLAY_GRID_W+ix];
}}

uint16_t kleiner_play_state_patterns(uint16_t state) BANKED {{
    if(state>=KLEINER_PLAY_STATE_COUNT)return 0u;
{chr(10).join(pattern_cases)}
    return 0u;
}}
...
void kleiner_play_upload_state(uint16_t state,uint8_t pool,
                            uint16_t first,uint16_t count) BANKED {{
    if(state>=KLEINER_PLAY_STATE_COUNT||!count)return;
{chr(10).join(upload_cases)}
}}

void kleiner_play_apply_name(uint16_t state,uint8_t pool) BANKED {{
    if(state>=KLEINER_PLAY_STATE_COUNT)return;
{chr(10).join(name_cases)}
}}

void kleiner_play_load_dictionary(void) BANKED {{
    if(KLEINER_PLAY_DICT_COUNT)
        set_bkg_4bpp_data(KLEINER_PLAY_DICT_BASE,KLEINER_PLAY_DICT_COUNT,k_dictionary);
}}
"""
    (outdir / "kleiner_playable_dispatch.c").write_text(s)
...
def main():
    ...
    print(
        f"KLEINER_PLAYABLE_C_PASS states={len(states)} banks={len(chunks)} "
