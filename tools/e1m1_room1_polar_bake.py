...
# Flat benchmark variant. Standard structural walls become one vertically
# symmetric floor-to-ceiling band around the fixed eye height. The two long
# north/south exterior walls become centered window walls: lower and upper
# bands leave a 4-world-unit aperture centred on eye z=5. XY topology and
# floor footprint are unchanged.
FLAT_WALL_Z0=0
FLAT_WALL_Z1=10
FLAT_WINDOW_Z0_TOP=3
FLAT_WINDOW_Z1_BOTTOM=7
FLAT_WINDOW_SOURCE_IDS=(19,46)
...
def flatten_segments(segs):
    out=[]
    full_flags=[]
    source_ids=[]
    for sid,(a,b,_z0,_z1,occ,bias) in enumerate(segs):
        # Drop floor steps, stair risers and ceiling/floor insets entirely.
        # Occluding structural boundaries and pillars keep their exact XY span.
        if not occ:
            continue
        if sid in FLAT_WINDOW_SOURCE_IDS:
            # Window aperture is symmetric about eye z=5: [3,7] is open.
            out.append((a,b,FLAT_WALL_Z0,FLAT_WINDOW_Z0_TOP,1,bias))
            full_flags.append(0)
            source_ids.append(sid)
            out.append((a,b,FLAT_WINDOW_Z1_BOTTOM,FLAT_WALL_Z1,1,bias))
            full_flags.append(0)
            source_ids.append(sid)
        else:
            out.append((a,b,FLAT_WALL_Z0,FLAT_WALL_Z1,1,bias))
            full_flags.append(1)
            source_ids.append(sid)
    if len(out)>64:
        raise SystemExit(f"flat segment set exceeds 64-bit PVS mask: {len(out)}")
    return out,full_flags,source_ids
...
def emit_meta(path,verts,segs,flat_flags=None,source_ids=None):
    v0=[s[0] for s in segs]; v1=[s[1] for s in segs]
    z0=[s[2] for s in segs]; z1=[s[3] for s in segs]
    bias=[s[5] for s in segs]
    if flat_flags is None:
        flat_flags=[0]*len(segs)
    if source_ids is None:
        source_ids=list(range(len(segs)))
    ...
    with path.open("w") as f:
        ...
        f.write(f"#define E1PF_VERTEX_COUNT {len(verts)}u\n#define E1PF_SEGMENT_COUNT {len(segs)}u\n")
        ...
        emit_list(f,"uint8_t","k_e1pf_flat_full",flat_flags)
        emit_list(f,"uint8_t","k_e1pf_source_sid",source_ids)
        f.write("#endif\n")
...
def main():
    ...
    ap.add_argument("--flat",action="store_true",
                    help="flatten floors and keep only structural FULL walls/pillars plus centered windows")
    args=ap.parse_args()
    ...
    source_ids=None
    if args.flat:
        segs,flat_flags,source_ids=flatten_segments(segs)
        # Keep exact walkable XY footprint, but remove every floor height.
        runs=[(x0,x1,0) for x0,x1,_z in runs]
    masks,counts,empty=bake_masks(verts,segs,offs,runs)

    out=Path(args.out_dir)
    out.mkdir(parents=True,exist_ok=True)
    emit_meta(out/"e1m1_room1_polar_meta.h",verts,segs,flat_flags,source_ids)
    ...
    if args.flat:
        full_count=sum(flat_flags)
        print(f"E1PF_FLAT_PASS segments={len(segs)} full={full_count} window_bands={len(segs)-full_count} windows={len(FLAT_WINDOW_SOURCE_IDS)} floor_z=0 wall_z=0..10 window_open_z=3..7")
