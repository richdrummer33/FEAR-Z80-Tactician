...
if 'STAGE18_COLUMN_LIFETIME' in raster:
    print('Stage 18 column-major lifetime state already applied')
    raise SystemExit(0)
if 'STAGE14_SPAN_COVERAGE' not in raster:
    raise SystemExit('apply Stage 14 span coverage first')

# Prefix mask n has rows [0,n) set across three bytes.
prefix = []
for n in range(19):
    mask = (1 << n) - 1 if n else 0
    prefix.append((mask & 0xff, (mask >> 8) & 0xff, (mask >> 16) & 0x03))
prefix_lines = []
for i,(a,b,c) in enumerate(prefix):
    prefix_lines.append(f'        .db 0x{a:02x},0x{b:02x},0x{c:02x} ; rows < {i}')

raster = raster.replace('; STAGE14_SPAN_COVERAGE\n',
                        '; STAGE14_SPAN_COVERAGE\n; STAGE18_COLUMN_LIFETIME\n',1)
raster_p.write_text(raster)

nt = r'''        .title  "TileSector Stage 18 column-major lifetime state"
...
nt_p.write_text(nt)
print('Applied Stage 18 column-major 18-bit lifetime masks.')
