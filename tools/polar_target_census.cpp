...
static std::vector<Range> load_polar_ranges(const char* noi,const char* sym) {
    ...
    while(std::getline(f,line)) {
        if(std::regex_search(line,m,re)) {
            ...
            if(a>=0x14000ul && a<0x18000ul) starts.push_back({(u16)a,m[1].str()});
    ...
    for(size_t i=0;i<starts.size();++i) {
        ...
        if(hi>lo) out.push_back({lo,hi,starts[i].second,0,0,0});
    }
    if(render_start && render_end>render_start)
        out.push_back({render_start,render_end,"tsp_polar_render(self)",0,0,0});

    u16 a=0;
    if(find_symbol(sym,"__mulint",a)) out.push_back({a,(u16)(a+2u),"__mulint(wrapper)",0,0,0});
    if(find_symbol(sym,"__mul16",a)) out.push_back({a,(u16)(a+0x15u),"__mul16",0,0,0});
    if(find_symbol(sym,".memset_simple",a)) out.push_back({a,(u16)(a+0x15u),"memset",0,0,0});
    if(find_symbol(sym,"__mullong",a)) out.push_back({a,(u16)(a+0x72u),"__mullong",0,0,0});
    ...
    for(size_t i=0;i<fixed.size();++i) {
        ...
        if(hi>lo) out.push_back({lo,hi,name.substr(1),0,0,0});
    }
    return out;
}

static bool press_scenario(GearsystemCore& core,const std::string& s) {
...
static void release_scenario(GearsystemCore& core,const std::string& s) {
    if(s=="roomA-turn") core.KeyReleased(Joypad_1,Key_Right);
    else if(s=="roomA-forward") core.KeyReleased(Joypad_1,Key_Up);
}

int main(int argc,char**argv) {
    if(argc<6) {
        std::fprintf(stderr,"usage: %s rom.gg rom.sym rom.noi mode scenario [updates=60] [warmup=8]\n",argv[0]);
        return 2;
    }
    const char* rom=argv[1];
    const char* sym=argv[2];
    const char* noi=argv[3];
    const unsigned wanted_mode=(unsigned)std::strtoul(argv[4],nullptr,0);
    const std::string scenario=argv[5];
    const unsigned target=(argc>6)?(unsigned)std::strtoul(argv[6],nullptr,0):60u;
    const unsigned warmup=(argc>7)?(unsigned)std::strtoul(argv[7],nullptr,0):8u;
    if(wanted_mode>2u || !target) return 2;

    u16 phase_addr=0,mode_addr=0;
    if(!any_symbol(sym,"_g_ts_prof_phase","g_ts_prof_phase",phase_addr) ||
       !any_symbol(sym,"_g_tspf_appearance_mode","g_tspf_appearance_mode",mode_addr)) {
    ...
    const uint64_t instruction_limit=250000000ull;
    uint64_t instructions=0;
    ...
    while(measured<target && instructions<instruction_limit) {
        ...
        const bool sample_render=armed && p_before==2u && loops>warmup;
        ...
        if(p!=last_phase) {
            if(p==1u) {
                if(!armed) {
                    mem->Write(mode_addr,(uint8_t)wanted_mode);
                    if(mem->DebugRetrieve(mode_addr)!=(uint8_t)wanted_mode) {
                        std::fprintf(stderr,"mode write failed\n"); return 6;
                    }
    ...
    release_scenario(core,scenario);
    if(measured<target) {
        std::fprintf(stderr,"only %u/%u updates captured after %llu instructions\n",measured,target,(unsigned long long)instructions);
        return 7;
    }
    ...
    std::printf("Polar render PC profile: mode=%u scenario=%s measured_updates=%u render_cycles=%llu render_instructions=%llu\n",
                wanted_mode,scenario.c_str(),measured,(unsigned long long)total_render_cycles,(unsigned long long)total_render_ins);
