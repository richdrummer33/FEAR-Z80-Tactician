...
echo "[1/3] Building Gearsystem libretro core..."
make -C "$ROOT/platforms/libretro" -j"$(nproc)"

echo "[2/3] Building libretro frame runner..."
g++ -std=c++17 -O2 -Wall -Wextra -Wpedantic   -I"$ROOT/platforms/libretro"   "$PORT/tools/libretro_runner.cpp" -ldl -o "$PORT/build/libretro_runner"

echo "[3/3] Building only the native phase profiler needed by E1M1 CI..."
...
g++ -std=c++17 -O2 -Wall -Wextra "${INC[@]}"   "$PORT/tools/tilesector_profile_runner.cpp"   "${ARGS[@]}" "$PORT/build/coredbg/emu2413.o" "$PORT/build/coredbg/miniz.o" -lm   -o "$PORT/build/tilesector_profile_runner"

echo "Built libretro_runner and tilesector_profile_runner."
