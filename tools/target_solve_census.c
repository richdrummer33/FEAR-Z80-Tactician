...
#define HN (1u << 22) // Hash Number: Alloccates hash table on host PC heap
static uint32_t *g_set;
static unsigned long g_setn;

// Open-addressed hash-set using linear probing to store & count unique vals.
// Maps 64 bit inputs into a fixed-size 32bit array to rec a "census" of unique states.
// Super sadge nooo
...
{
    memset(g_set, 0, HN * sizeof(uint32_t));
    g_setn = 0;
}
...
int main(int argc, char **argv)
{
    ...
    for (gy = 0; gy < GRID_H; ++gy)
        for (gx = 0; gx < GRID_W; ++gx)
        {
            ...
            if (!tsp_is_walkable_q4(px0, py0))
                continue;
            if (k_tspf_recipe_grid[gi0] == 0xffu)
                continue;
            for (oi = 0; oi < 4; ++oi)
            {
                int16_t px = (int16_t)(px0 + off[oi][0]), py = (int16_t)(py0 + off[oi][1]);
                if (!tsp_is_walkable_q4(px, py))
                    continue;
                for (yaw = 0; yaw < 256u; yaw += yaw_step)
                {
                    ...
                    for (j = 0; j < nk; ++j)
                    {
                        if (count >= TSPF_MAX_ACTIVE)
                            break;
                        if (!project_key(ks[j], &s, &g_runs[count]))
                            continue;
                    ...
                    for (i = 0; i < count; ++i)
                    {
                        ...
                        if (dp_solve(rr->sid, invd, c0, c1, &t))
                        {
                            ...
                            sadd(((uint64_t)(uint16_t)t.step << 16) | (uint32_t)(((a % 1024) + 1024) % 1024));
