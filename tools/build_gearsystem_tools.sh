...
echo "[1/7] Building Gearsystem libretro core (no SDL required)..."
make -C "$ROOT/platforms/libretro" -j"$(nproc)"

echo "[2/7] Building tiny libretro frame runner..."
g++ -std=c++17 -O2 -Wall -Wextra -Wpedantic \
  -I"$ROOT/platforms/libretro" \
  "$PORT/tools/libretro_runner.cpp" -ldl -o "$PORT/build/libretro_runner"

echo "[3/7] Building native Gearsystem core debugger runner..."
