...
CELL=16
COLS=(WORLD_MAX_X-WORLD_MIN_X+CELL-1)//CELL
ROWS=(WORLD_MAX_Y-WORLD_MIN_Y+CELL-1)//CELL
YAW_BINS=16
MASK_BYTES=4
...
def bake_pvs(walk):
    ...
    for gy in range(ROWS):
      for gx in range(COLS):
        ...
        pts=((x0+CELL*.5,y0+CELL*.5),(x0+.5,y0+.5),(x0+CELL-.5,y0+.5),(x0+.5,y0+CELL-.5),(x0+CELL-.5,y0+CELL-.5))
...
def main():
    ...
    sections=["/* GENERATED by tools/full_maze_bake.py. */",
      ...
      f"#define FULLMAZE_PVS_COLS {COLS}u",f"#define FULLMAZE_PVS_ROWS {ROWS}u",f"#define FULLMAZE_PVS_CELL_SHIFT {int(math.log2(CELL))}u",f"#define FULLMAZE_PVS_YAW_BINS {YAW_BINS}u",f"#define FULLMAZE_PVS_MASK_BYTES {MASK_BYTES}u",
